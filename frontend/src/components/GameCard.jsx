...
export const GameCard = (props) => {
  ...
  const [likes, setLikes] = useState(() => Number(localStorage.getItem(likesKey)) || 0);
  const [dislikes, setDislikes] = useState(() => Number(localStorage.getItem(dislikesKey)) || 0);

  useEffect(() => { localStorage.setItem(likesKey, likes); }, [likes, likesKey]);
  ...
  const handleSaveForLater = async () => {
    ...
    try {
      ...
      if (result && result.success) {
        ...
        dispatch({ type: "save_for_later", payload: gameData });
      } else {
  ...
  return (
    ...
    >
      <div className="card-body">
        <Link to={`/retrogame/${props.uid}`} style={{ textDecoration: "none", color: "inherit" }}>
          <img
            src={getImageSrc()}
            className="card-img-top"
            alt={props.name || "Game Image"}
            onError={(e) => { e.target.onerror = null; e.target.src = "https://t3.ftcdn.net/jpg/04/60/01/36/360_F_460013622_6xF8uN6ubMvLx0tAJECBHfKPoNOR5cRa.jpg"; }}
            style={{ height: "300px", width: "100%", objectFit: "cover", borderRadius: ".5rem" }}
          />
        </Link>
        ...
        <div className="d-flex justify-content-center gap-2 mt-2">
          <button className="btn btn-outline-success" onClick={() => setLikes((prev) => prev + 1)}>👍 {likes}</button>
          <button className="btn btn-outline-danger" onClick={() => setDislikes((prev) => prev + 1)}>👎 {dislikes}</button>
          <button
